...
{
    class Competitive_Sale_Result
    {
        ExtentTest test = ExtentReport.ReportStart("Competitive Sale Results", "Competitive Sale Results Functionality Under Primary Market");

        public void checkingdata(IWebDriver driver)
        {
            List<IWebElement> list_competitive_link = new List<IWebElement>();
            List<IWebElement> list_competitive_link1 = new List<IWebElement>();
            ...
            ExtentReport.ReportLog(test, "Info", "Number of News Links Under Competitive Sale Results" + "  " + count + "  " + "Starting from date" +
            ...
            try
            {
                for (int j = 0; j < num; j++)
                {
                    list_competitive_link[j].Click();
                    Thread.Sleep(TimeSpan.FromSeconds(5));
                    ExtentReport.ReportLog(test, "Pass", "Clicked on Links under Competitive Sale Results", driver);
            ...
            catch
            {
                ...
                List<IWebElement> list_competitive_link11 = new List<IWebElement>();
                ...
                for (int j = 0; j < num; j++)
                {
                    list_competitive_link11[j].Click();
                    Thread.Sleep(TimeSpan.FromSeconds(5));
                    ExtentReport.ReportLog(test, "Pass", "Clicked on Links under Competitive Sale Results", driver);
                    IWebElement elemen = driver.FindElement(By.Id("closeOverlay"));
                    IJavaScriptExecutor js = (IJavaScriptExecutor)driver;
